...
packages <- c("data.table", "skimr", "tidyverse", "lubridate", "dplyr", "ggplot2", "readr")  # Add packages you need
...
source(file.path(scripts_dir, "01-load.R"))  # Assuming script1 is for loading data

cat("Running script 2: Data preprocessing...\n")
source(file.path(scripts_dir, "02-tidy.R"))  # Assuming script2 is for processing data

cat("Running script 3: Data analysis...\n")
source(file.path(scripts_dir, "03-analysis.R"))  # Assuming script3 is for analysis
